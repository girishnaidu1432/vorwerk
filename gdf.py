        ...
        try:
            ...
            for i, col in enumerate(chart_cols):
                with cols[i]:
                    ...
                    sns.barplot(ax=ax, x=col, y='Predicted_Qty', data=df_gen_yes, palette="viridis")
                    ax.set_title(f'Avg Qty by {col}', fontsize=10)
                    ax.set_xlabel("")
                    ax.set_ylabel("Predicted Qty", fontsize=8)
